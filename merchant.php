	...
	public function insert() {
		...
		if ($step == 1) {
			...
			if ($count_staff_mobile != 0) {
				$this->showmessage('该手机号码已存在', ecjia::MSGTYPE_JSON | ecjia::MSGSTAT_ERROR);
			}
...
// 				'identity_status'			=> $identity_status,		//证件认证状态，0待审核，1审核中，2审核通过，3拒绝通过
		...
		if ($step == 1) {
			...
			if (!empty($code) && $code == $_SESSION['temp_code'] && $time < $_SESSION['temp_code_time'] && $mobile == $_SESSION['temp_mobile']) {
				...
				if (empty($count_preaudit_info) && empty($count_franchisee_info)) {
					$this->showmessage('没有关于该手机号的入驻信息', ecjia::MSGTYPE_JSON | ecjia::MSGSTAT_ERROR);
				}
	...
	private function add_check_log($data, $store_info = array(), $insert_id = 0) {
	    //审核日志
	    //add
	        ...
	        foreach ($edit_fields as $field_key => $field_name) {
	            if ($store_info[$field_key] != $data[$field_key]) {
	                if ($field_key == 'cat_id') {
	                    ...
	                    $store_info[$field_key] = $store_info[$field_key] ? RC_Upload::upload_url($store_info[$field_key]) : '<空>';
	                    $data[$field_key] = $data[$field_key] ? RC_Upload::upload_url($data[$field_key]) : '<空>';
	                }
